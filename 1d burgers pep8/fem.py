...
def get_prediction_indexes(results, indexes):
    ...
    pred = [results[0][index] for index in indexes]
    return pred
...
def get_gradient(l2_lambda, indexes, results, y_train, control_variable, 
                solution_comparison, map):
...
def burgers_1d(viscosity, initial_condition, gradient_mode=False, excluded_indices = []):
    """Inspired by:
    https://people.math.sc.edu/Burkardt/fenics_src/burgers_time_viscous/burgers_time_viscous.html."""
    ...
    F = (
        dot(cur_vel - old_vel, test_vel) / dt 
        + viscosity * inner(grad(cur_vel), grad(test_vel)) 
        + inner(cur_vel * cur_vel.dx(0), test_vel)
    ) * dx
...
5. def get_prediction_indexes(results, indexes):
    pred = []
