...
3. for i in range(0, *): --> for i in range(*): (it is the same thing)

4. def prepare_tensor(data):
    tensor_data = []
    for entry in data:
...
5. def add_noise(data, noise_level=0):
    noisy_data = []
    for entry in data:
...
6. def create_ic_data():
    x_ic, y_ic = [], []
    for i in range(160):
        x = np.random.uniform(-1, 1)
        x_ic.append(np.array([x, 0]))
        y_ic.append(np.array([-1*np.sin(x*np.pi)])) --> y_ic.append(np.array([-np.sin(x*np.pi)]))

7. two blank lines between functions
