...
def tgv_vortex(visc, slsqp=[], pinn=[]):
    ...
    for t in tqdm(range(max_step), desc=f"Rank {rank}", position=rank, ncols=70, leave=False):
        ...
        if len(slsqp) > 0:
            ...
            time_filtered = [
                ...
                if np.isclose(point[2].detach().cpu().numpy() * time_scale, t + 1)
            ] 
            for idx, point in time_filtered:
                x, y = point[0], point[1]
                try:
                    x_vel, y_vel = u_next(x, y)
                    pressure = p_next(x, y) - mean_pressure
                    local_predictions.append((idx, [x_vel, y_vel, pressure]))
                except Exception as e:
                    print(f"[Rank {rank}] Evaluation failed at ({x:.3f}, {y:.3f}) β†’ {e}", flush=True)

        u_prev.assign(u_next)

    np.save(f"predictions_rank{rank}.npy", np.array(local_predictions, dtype=object))
    ...
    if rank == 0:
        ...
        """
        np.save(f"allpredictions.npy", np.array(all_predictions, dtype=object))
        combined = []
